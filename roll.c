			...
			switch(argv[i][1])
			{
				...
				print_help:
					fprintf(stderr,
						...
						"	         May only appear once per action, unless only a single value matches the case.\n"
						"	n      A constant value of n. n may be 0.\n"
						"	D~P    Rerolls once if the pattern is hit.\n"
						"	D\\P   Like ~ with infinite rerolls.\n"
						...
						" Where n and m represent positive whole numbers, D represents another die, and P a pattern.\n"
						" Operator precedence is as shown.\n"
						"Sets:\n"
						" A set is a list of ranges, which are either single numbers, or two numbers separated by '-'\n"
						" These numbers may be negative and may be replaced with '*' to indicate that that side of the range is open-ended.\n"
						" Instead of a range, the special values '^' or '_' can be used, which reference the highest or lowest possible value, respectively.\n"
						" Prefixing a set with '!' negates it so that every value not listed is rerolled.\n"
						"Patterns:\n"
